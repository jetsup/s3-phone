...
void ui_settingsNetworkWiFiPasswordScreen_screen_init(void) {
  ...
  lv_obj_set_align(ui_txtWiFiPassword, LV_ALIGN_TOP_LEFT);
  lv_obj_set_width(ui_txtWiFiPassword, 205);
  lv_obj_set_height(ui_txtWiFiPassword, 35);
  lv_obj_set_y(ui_txtWiFiPassword, yOffset);
  lv_obj_set_x(ui_txtWiFiPassword, UI_ALIGN_TOP_LEFT_X_OFFSET + 5);
  ...
  // add password view toggle eye
  lv_obj_t *imgPasswordView =
      lv_label_create(ui_panelSettingsNetworkWiFiPassword);
  lv_label_set_text(imgPasswordView,
                    LV_SYMBOL_EYE_OPEN);  // Initial icon (open eye)
  lv_obj_align_to(imgPasswordView, ui_txtWiFiPassword, LV_ALIGN_OUT_RIGHT_MID,
                  5,
                  0);  // Position to the right of text area
  lv_obj_add_flag(imgPasswordView, LV_OBJ_FLAG_CLICKABLE);  // Make it clickable
  lv_obj_add_event_cb(imgPasswordView, toggle_password_visibility_cb,
                      LV_EVENT_CLICKED, ui_txtWiFiPassword);
