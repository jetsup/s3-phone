...
void ui_extraScreen_screen_init(void) {
  ui_extraScreen = lv_obj_create(NULL);
  lv_obj_remove_flag(ui_extraScreen, LV_OBJ_FLAG_SCROLLABLE);  /// Flags

  ui_panelExtraMain = lv_obj_create(ui_extraScreen);
  ...
  lv_obj_set_style_bg_image_opa(ui_panelExtraMain, 100,
                                LV_PART_MAIN | LV_STATE_DEFAULT);
  lv_obj_set_style_border_color(ui_panelExtraMain, lv_color_hex(0x000000),
                                LV_PART_MAIN | LV_STATE_DEFAULT);
  lv_obj_set_style_border_opa(ui_panelExtraMain, 0,
                              LV_PART_MAIN | LV_STATE_DEFAULT);

  ui_add_bottom_bar(ui_panelExtraMain, 0xFFFFFF, 10);
}
